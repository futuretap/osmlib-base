...
module OSM
  ...
  class API
    ...
    DEFAULT_BASE_URI = 'https://www.openstreetmap.org/api/0.6/'
    ...
    def get_object(type, id)
      ...
      if type == 'way'
        way = list.last
        way.node_objects = way.nodes.map{|node_id| list.select{|node| node.id == node_id.to_i}.first}
        return way
      end
    ...
    def get(suffix)
      ...
      request.use_ssl = uri.is_a?(URI::HTTPS)
